...
export default async function DashboardPage() {
  ...
  const [applicationsResult, todayResult, appliedResult, failedResult] = await Promise.all([
    supabase
      .from("applications")
      .select("*")
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="border rounded-lg p-4 text-center">
          <div className="text-3xl font-bold text-blue-600">{todayResult.count ?? 0}/{session.user.dailyLimit}</div>
          <div className="text-sm text-gray-500 mt-1">Applied Today</div>
        </div>
        <div className="border rounded-lg p-4 text-center">
          <div className="text-3xl font-bold text-green-600">{appliedResult.count ?? 0}</div>
          <div className="text-sm text-gray-500 mt-1">Total Applied</div>
        </div>
        <div className="border rounded-lg p-4 text-center">
          <div className="text-3xl font-bold text-red-600">{failedResult.count ?? 0}</div>
